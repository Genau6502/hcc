        ...
        , [   
            ...
            , "(2) Single declare and assign statement" -: 
                ...
                in
                    ...
                    --> ([MOV L (Immediate 0) R13,MOV L R13 R12], 0, 0)
            , "(3) Sequential statements with live variable carried over" -: 
                ...
                in
                    compileBlock lvs emptyRA 0 stmts 
                    --> ([MOV L (Immediate 2) R13, MOV L R13 R12,MOV L R12 R13], 0, 0)
            ...
            , "(5) Statement requiring two dummy variables (binary expression)" -: compileBlock [] emptyRA 0 [DeclareAndAssignStmt (Var IntType "z") (AddExpr (IntAtom 2) (IntAtom 3))] --> ([MOV L (Immediate 2) R13, MOV L (Immediate 3) R14, ADD L R13 R14 R15, MOV L R15 R12], 0, 0)
            , "(6) Subtraction with two literals" -: compileBlock [] emptyRA 0 [DeclareAndAssignStmt (Var IntType "x") (SubtractExpr (IntAtom 5) (IntAtom 3))] --> ([MOV L (Immediate 5) R13, MOV L (Immediate 3) R14, SUB L R13 R14 R15, MOV L R15 R12], 0, 0)
            , "(7) Multiplication of literal and live variable" -: compileBlock [Var IntType "x"] (makeRA [(Var IntType "x", R12)]) 0 [DeclareAndAssignStmt (Var IntType "y") (MultiplyExpr (IntAtom 4) (VarAtom (Var IntType "x")))] --> ([MOV L (Immediate 4) R14, IMUL L R14 R12 R15, MOV L R15 R13], 0, 0)
            ...
            , "(13) Assignment with arithmetic expression (x = y + 3;)" -: compileBlock [Var IntType "x", Var IntType "y"] (makeRA [(Var IntType "x", R12), (Var IntType "y", R13)]) 0 [ExprStmt (AssignExpr (Var IntType "x") (AddExpr (VarAtom (Var IntType "y")) (IntAtom 3)))] --> ([MOV L (Immediate 3) R14, ADD L R13 R14 R15, MOV L R15 R12], 0, 0)
            ...
            , "(16) Return an arithmetic expression (return x + 5;)" -: compileBlock [Var IntType "x"] (makeRA [(Var IntType "x", R12)]) 0 [ReturnStmt (AddExpr (VarAtom (Var IntType "x")) (IntAtom 5))] --> ([MOV L (Immediate 5) R13, ADD L R12 R13 R14, MOV L R14 RAX, RET_PLA], 0, 0)
        ]
    )
...
compileFunctionTests = TestGroup 
    (
        ...
        , [   
              "(1) Simple main function returning literal: int main() { return 1; }" -: 
                let 
                    f = Function "main" [] IntType [ReturnStmt (AtomExpr (IntAtom 1))]
                    startLabel = 0
                in
                    compileFunction f startLabel
                    -- maxStackSize is 0, so no RSP shifting. 
                    -- IntAtom 1 uses R12, then moves to RAX. RET_PLA is replaced with RET.
                    --> ([ FuncLabel "main"
                         , MOV L (Immediate 1) R12
                         , MOV L R12 RAX
                         , RET
            ...
            , "(2) Function with 1 argument: int identity(int x) { return x; }" -: 
                let 
                    vX = Var IntType "x"
                    f = Function "identity" [vX] IntType [ReturnStmt (AtomExpr (VarAtom vX))]
                    startLabel = 0
                in
                    compileFunction f startLabel
                    -- The argument 'x' is perfectly mapped to RDI. 
                    -- Returns RDI through RAX.
                    --> ([ FuncLabel "identity"
                         , MOV L RDI RAX
                         , RET
                         ], 0)

            , "(3) Function with 2 arguments and arithmetic: int add(int a, int b) { return a + b; }" -: 
                let 
                    vA = Var IntType "a"
                    vB = Var IntType "b"
                    f = Function "add" [vA, vB] IntType [ReturnStmt (AddExpr (VarAtom vA) (VarAtom vB))]
                    startLabel = 0
                in
                    compileFunction f startLabel
                    -- 'a' maps to RDI, 'b' maps to RSI. 
                    -- ADD allocates dummy R12 to store the result, then moves to RAX.
                    --> ([ FuncLabel "add"
                         , ADD L RDI RSI R12
                         , MOV L R12 RAX
                         , RET
            ...
            , "(4) Function with empty body (compiles cleanly without crashing)" -: 
                let 
                    f = Function "empty" [] VoidType []
                    startLabel = 0
                in
                    compileFunction f startLabel
                    --> ([ FuncLabel "empty" ], 0)
